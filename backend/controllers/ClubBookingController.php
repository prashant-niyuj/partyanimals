       ...
       // var_dump($arr_booking_data);die;
		...
		if(isset($arr_booking_data['c_id'])) {
			...
			else
			{
				...
				if(strtolower($arr_booking_data['b_type']) == 'group') 
				{
					$model->no_of_girls = $arr_booking_data['popup_drop_girls'];
					$model->no_of_boys = $arr_booking_data['popup_drop_boys'];
				}else if(strtolower($arr_booking_data['b_type']) == 'girl')
                                {
                                    $model->no_of_girls = 1;
                                }else if(strtolower($arr_booking_data['b_type']) == 'boy')
                                {
                                    $model->no_of_boys = 1;
                                }else if(strtolower($arr_booking_data['b_type']) == 'couple')
                                {
                                    $model->no_of_boys = 1;
                                    $model->no_of_girls =1;
                                }
					...
					if($modelBookingPaymentHistory->save())
					{
						...
						if(Yii::$app->params['is_local']) {
				            ...
				            try {
		                        ...
		                        'redirect_url' => 'http://partyanimals.in/beta/backend/web/index.php?r=club-booking/thankyou&b_id='.$order_id
		                        ));
