...
if(!Yii::$app->user->isGuest)
{
   ...
   $profile= app\models\Profile::findOne($userinfo['id']);
...
<div class="partyanimals">
	...
	<form method="post" id="booking_confirm" action="index.php?r=booking/processorder">
    <input type="hidden" name="<?= Yii::$app->request->csrfParam; ?>" value="<?= Yii::$app->request->csrfToken; ?>" />		
	<h2 class="confirm_h2">Confirm Booking</h2>
    <h3 class="confirm_h4"> <?php echo $obj_club_info->name." <span>(".ucfirst($obj_club_info->area)."- Pune)</span> "; ?></h3>
	<p class="text-center">
			<?php
			if(isset($arr_data['txt_datepicker']))
	 		...
	 		<!--a href="#" style="padding-left:10px"> <span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></a-->
	</p>
	<div id="isconfirm">
	<div class="alert alert-danger isfull" role="alert" style="display:none">Sorry! Club has Allready full</div>
		
	  <table class="table table-condensed detailpage">
        ...
        <tr>
		  ...
		  <table width="100%">
			...
			<tr>
				...
				<div class="col-xs-6 col-md-6">
				<div class="form-group">				  
				 <input name="booking_person[]" type="text" placeholder="Mr." class="form-control " value="<?php echo ($i == 0 ) ? $arr_data['name'] : '' ?>">
				  </div>
				</div>
				<div class="col-xs-6 col-md-6">
				<div class="form-group">
				  <?php if($i == 0) { 
					$is_phone_set = 1;
				  ?>	
				  <input id="booking_person_mobile_0" name="booking_person_mobile[]" type="text" placeholder="Mobile No" class="form-control " value="<?php echo ($i == 0 ) ? $arr_data['phone'] : '' ?>"> 
				  <?php } else { ?>
					<input name="booking_person_mobile[]" type="text" placeholder="Mobile No" class="form-control " > 
				  <?php } ?>	
				</div>
	   ...
	   <?php if($obj_club_info->tax_rate > 0 )
        ...
        <tr>
          <td colspan="2">
          	<div class="form-group">
			    <div class="checkbox">
			      <label>&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" id="agree" name="agree" value="agree" />I Accept <a href="index.php?r=site/termsandconditions">Terms & condition</a></label>
			    </div>
			</div>
		  </td>
        </tr>
    ...
    .on("success.form.bv", function(e) {
        ...
        $.post($form.attr("action"), $form.serialize(), function(result) {
			...
			else 
			{
				//alert("sucess");
				if(result["b_id"])
					//window.location.href = "index.php?r=site/thankyou&b_id="+result["b_id"];
